import { supabase } from "../../../dashboard/js/supabase.js";
import { recordStreak } from "../../../dashboard/pages/streak.js";

const wsubject = sessionStorage.getItem("wsubject");
const wsnq = sessionStorage.getItem("wsnq");
const wtime = sessionStorage.getItem("wtime");
const WexamType = sessionStorage.getItem("WexamType");
const loadimg = document.getElementById("loader");

const loader = document.getElementById("loading");
const reload = document.getElementById("reload");
const linfo = document.getElementById("linfo");
const tawinfo = document.getElementById("tawinfo");
const nextbtn = document.getElementById("nextbtn");

let isSubmitted = false;
let isSavingSession = false;

const API_URL = `https://questions.aloc.com.ng/api/v2/q?subject=${wsubject}&type=UTME&limit=${wsnq}`;
const API_KEY = "ALOC-042dc5afca780faf2da4";

let questions = [];
let currentQuestion = 0;
let userAnswers = [];
let startTime = Date.now();

async function loadQuestions() {
  try {
    if (!wsubject || !wsnq) {
      throw new Error("Practice information is missing.");
    }

    const questionCount = Number(wsnq);

    if (!questionCount || questionCount <= 0) {
      throw new Error("Invalid question count.");
    }

    const requests = Array.from({ length: questionCount }, () =>
      fetch(API_URL, {
        method: "GET",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
          AccessToken: API_KEY,
        },
      }).then(async (res) => {
        if (!res.ok) {
          throw new Error(`API request failed: ${res.status}`);
        }

        return res.json();
      }),
    );

    const results = await Promise.all(requests);

    questions = results
      .map((result) => result.data)
      .filter((question) => question);

    if (!questions.length) {
      throw new Error("No questions were returned.");
    }

    timebox();
    createQuestionBoxes();
    showQuestion();
    hideLoader();
  } catch (err) {
    console.error("Error loading questions:", err);

    showToast("Failed to load questions, try again", "error");

    if (linfo) {
      linfo.textContent = "Failed Loading Questions";
      loadimg.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="150px" height="80.719px" viewBox="0 0 800 430.493" xmlns:xlink="http://www.w3.org/1999/xlink" role="img" artist="Katerina Limpitsouni" source="https://undraw.co/"><g transform="translate(-368.866 -254.661)"><ellipse cx="149.199" cy="26.107" rx="149.199" ry="26.107" transform="translate(368.866 481.453)" fill="#090814"/><ellipse cx="191.906" cy="26.107" rx="191.906" ry="26.107" transform="translate(785.054 632.939)" fill="#090814"/><path d="M527.508,703.017a139.876,139.876,0,0,1-34.251-3.807l1.007-3.942c61.693,15.766,176.375-20.484,188.785-59.673l3.879,1.228C676.145,670.877,591.731,703.017,527.508,703.017Z" transform="translate(229.585 -61.587)" fill="#2f2e41"/><path d="M422.29,529.663c-14.453-3.693-21.922-7.965-24.218-13.849-2.856-7.316,2.5-15.561,9.919-26.972,11.315-17.409,26.811-41.252,24.3-77.646-1.418-20.568-8.78-35.7-21.879-44.978-26.656-18.878-68.707-7.567-69.129-7.451l-1.085-3.921c1.8-.5,44.3-11.949,72.551,8.041,14.153,10.017,22.094,26.176,23.6,48.028,2.6,37.751-13.977,63.262-24.949,80.143-6.756,10.394-11.637,17.9-9.54,23.275,1.748,4.477,8.559,8.1,21.436,11.386Z" transform="translate(278.852 29.977)" fill="#2f2e41"/><path d="M816.039,661.123,795.384,696.05s-26.3,13.971-7.807,14.382,105.6,0,105.6,0,16.847,0-9.862-14.793l-19.389-37.478Z" transform="translate(136.968 -68.851)" fill="#e6e6e6"/><rect width="348.044" height="242.439" rx="18.046" transform="translate(801.887 355.38)" fill="#e6e6e6"/><path d="M2.712,0h319.2a2.712,2.712,0,0,1,2.712,2.712V193.294a2.712,2.712,0,0,1-2.712,2.712H2.712A2.712,2.712,0,0,1,0,193.294V2.712A2.712,2.712,0,0,1,2.712,0Z" transform="translate(813.598 368.323)" fill="#b00222"/><circle cx="2.055" cy="2.055" r="2.055" transform="translate(973.854 360.516)" fill="#f2f2f2"/><path d="M957.868,621.322v21.252a12.236,12.236,0,0,1-12.239,12.237H622.061a12.236,12.236,0,0,1-12.237-12.239V621.322Z" transform="translate(192.063 -56.993)" fill="#e6e6e6"/><path d="M864.04,660.444v4.109H642.968v-3.288l.3-.822,5.449-14.793h210.8Z" transform="translate(202.681 3.326)" fill="#f2f2f2"/><path d="M1096.367,755.317c-.4,1.722-1.923,3.538-5.358,5.256-12.327,6.164-37.393-1.644-37.393-1.644s-19.313-3.287-19.313-11.917a15.428,15.428,0,0,1,1.693-1.007c5.183-2.742,22.367-9.508,52.837.286a12.709,12.709,0,0,1,5.792,3.8C1095.859,751.534,1096.828,753.367,1096.367,755.317Z" transform="translate(55.429 -95.57)" fill="#f2f2f2"/><path d="M1097.171,755.317c-15.089,5.782-28.538,6.213-42.34-3.374-6.961-4.832-13.285-6.028-18.031-5.938,5.183-2.742,22.367-9.508,52.837.286a12.708,12.708,0,0,1,5.792,3.8C1096.663,751.534,1097.631,753.367,1097.171,755.317Z" transform="translate(54.626 -95.57)" fill="#e6e6e6"/><path d="M864.04,667.466v4.109H642.968v-3.288l.3-.822Z" transform="translate(202.681 -3.696)" opacity="0.1"/><path d="M794.285,488.635H592.452a6.086,6.086,0,0,1-6.079-6.079V414.467a6.086,6.086,0,0,1,6.079-6.079H794.285a6.086,6.086,0,0,1,6.079,6.079v68.088A6.086,6.086,0,0,1,794.285,488.635Z" transform="translate(-181.314 -153.727)" fill="#b00222"/><rect width="53.498" height="6.079" transform="translate(436.671 272.899)" fill="#f2f2f2"/><circle cx="3.648" cy="3.648" r="3.648" transform="translate(557.041 272.899)" fill="#f2f2f2"/><circle cx="3.648" cy="3.648" r="3.648" transform="translate(569.2 272.899)" fill="#f2f2f2"/><circle cx="3.648" cy="3.648" r="3.648" transform="translate(581.358 272.899)" fill="#f2f2f2"/><path d="M794.285,601.689H592.452a6.087,6.087,0,0,1-6.079-6.079V527.521a6.086,6.086,0,0,1,6.079-6.079H794.285a6.086,6.086,0,0,1,6.079,6.079v68.088A6.086,6.086,0,0,1,794.285,601.689Z" transform="translate(-181.314 -180.455)" fill="#e6e6e6"/><rect width="53.498" height="6.079" transform="translate(436.671 359.225)" fill="#b00222"/><circle cx="3.648" cy="3.648" r="3.648" transform="translate(557.041 359.225)" fill="#b00222"/><circle cx="3.648" cy="3.648" r="3.648" transform="translate(569.2 359.225)" fill="#b00222"/><circle cx="3.648" cy="3.648" r="3.648" transform="translate(581.358 359.225)" fill="#b00222"/><path d="M794.285,714.743H592.452a6.087,6.087,0,0,1-6.079-6.079V640.575a6.086,6.086,0,0,1,6.079-6.079H794.285a6.086,6.086,0,0,1,6.079,6.079v68.089A6.086,6.086,0,0,1,794.285,714.743Z" transform="translate(-181.314 -207.183)" fill="#e6e6e6"/><rect width="53.498" height="6.079" transform="translate(436.671 445.552)" fill="#b00222"/><circle cx="3.648" cy="3.648" r="3.648" transform="translate(557.041 445.552)" fill="#b00222"/><circle cx="3.648" cy="3.648" r="3.648" transform="translate(569.2 445.552)" fill="#b00222"/><circle cx="3.648" cy="3.648" r="3.648" transform="translate(581.358 445.552)" fill="#b00222"/><g transform="translate(933.489 421.851)"><path d="M0,0H88.949V88.949H0Z" fill="none"/><path d="M39.062,2A37.062,37.062,0,0,1,76.125,39.062a37.06,37.06,0,1,1-74.11,1.2L2,39.062l.015-1.038A37.061,37.061,0,0,1,39.062,2ZM39.1,50.181l-.471.026a3.706,3.706,0,0,0,0,7.361l.434.026.471-.026a3.706,3.706,0,0,0,0-7.361Zm-.037-32.362a3.706,3.706,0,0,0-3.68,3.272l-.026.434v20.25l.026.434a3.706,3.706,0,0,0,7.361,0l.026-.434V21.525l-.026-.434A3.706,3.706,0,0,0,39.062,17.819Z" transform="translate(5.412 5.412)" fill="#f2f2f2"/></g></g></svg>`;
    }

    if (tawinfo) {
      tawinfo.style.display = "none";
    }

    if (reload) {
      reload.style.display = "flex";
    }
  }
}

function hideLoader() {
  if (!loader) return;

  loader.classList.add("remove");
  loader.style.display = "none";
}

function createQuestionBoxes() {
  const container = document.getElementById("questbox");

  if (!container) return;

  container.innerHTML = "";

  for (let i = 0; i < questions.length; i++) {
    const div = document.createElement("div");

    div.textContent = i + 1;
    div.classList.add("qboxes");

    div.onclick = () => {
      if (isSubmitted) return;

      saveAnswer();
      currentQuestion = i;
      showQuestion();
    };

    container.appendChild(div);
  }
}

function updateBoxes() {
  const boxes = document.querySelectorAll(".qboxes");

  boxes.forEach((box, i) => {
    box.classList.remove("active", "answered");

    if (i === currentQuestion) {
      box.classList.add("active");
    } else if (userAnswers[i]) {
      box.classList.add("answered");
    }
  });
}

function showQuestion() {
  const q = questions[currentQuestion];

  if (!q) return;

  const questionbody = document.getElementById("questionbody");

  if (!questionbody) return;

  const html = `
    <h2>
      Question <b>${currentQuestion + 1}</b> of <b>${questions.length}</b>
    </h2>

    <p>${q.question}</p>

    <div class="options">
      <div>
        <input type="radio" name="option" value="a">
        <label>${q.option.a}</label>
      </div>

      <div>
        <input type="radio" name="option" value="b">
        <label>${q.option.b}</label>
      </div>

      <div>
        <input type="radio" name="option" value="c">
        <label>${q.option.c}</label>
      </div>

      <div>
        <input type="radio" name="option" value="d">
        <label>${q.option.d}</label>
      </div>
    </div>
  `;

  questionbody.innerHTML = html;

  if (currentQuestion === questions.length - 1) {
    nextbtn.textContent = "Submit";
  } else {
    nextbtn.textContent = "Next Question";
  }

  if (userAnswers[currentQuestion]) {
    const selectedInput = document.querySelector(
      `input[value="${userAnswers[currentQuestion]}"]`,
    );

    if (selectedInput) {
      selectedInput.checked = true;
    }
  }

  updateBoxes();
}

function nextQuestion() {
  if (isSubmitted) return;

  saveAnswer();

  if (currentQuestion < questions.length - 1) {
    currentQuestion++;
    showQuestion();
  } else {
    submitTest();
  }
}

function prevQuestion() {
  if (isSubmitted) return;

  saveAnswer();

  if (currentQuestion > 0) {
    currentQuestion--;
    showQuestion();
  }
}

function saveAnswer() {
  const selected = document.querySelector('input[name="option"]:checked');

  if (selected) {
    userAnswers[currentQuestion] = selected.value;
  }
}

async function savePracticeSession(score, timeTaken) {
  if (isSavingSession) return;

  isSavingSession = true;

  try {
    const {
      data: { user },
      error: userError,
    } = await supabase.auth.getUser();

    if (userError) {
      throw userError;
    }

    if (!user) {
      throw new Error("No authenticated user found.");
    }

    const percentage = questions.length
      ? Math.round((score / questions.length) * 100)
      : 0;

    const { error: sessionError } = await supabase
      .from("practice_sessions")
      .insert({
        user_id: user.id,
        subject: wsubject,
        correct_answers: score,
        total_questions: questions.length,
      });

    if (sessionError) {
      throw sessionError;
    }

    const { error: historyError } = await supabase
      .from("practice_history")
      .insert({
        user_id: user.id,
        subject: wsubject,
        exam_type: WexamType,
        total_questions: questions.length,
        correct_answers: score,
        score_percentage: percentage,
        time_taken: timeTaken,
      });

    if (historyError) {
      console.error("Failed to save practice history:", historyError);

      showToast(
        "Progress saved, but practice history could not be saved.",
        "error",
      );

      await recordStreak();

      return;
    }

    await recordStreak();

    showToast("Progress saved successfully.", "success");
  } catch (error) {
    console.error("Failed to save practice session:", error);

    showToast(
      "Practice finished, but your progress could not be saved.",
      "error",
    );
  } finally {
    isSavingSession = false;
  }
}

async function submitTest() {
  if (isSubmitted) return;

  isSubmitted = true;

  saveAnswer();

  let score = 0;

  questions.forEach((q, i) => {
    if (userAnswers[i] === q.answer) {
      score++;
    }
  });

  const endTime = Date.now();
  const timeTaken = Math.floor((endTime - startTime) / 1000);

  function formatTime(sec) {
    const m = Math.floor(sec / 60);
    const s = sec % 60;

    return `${m}m ${s}s`;
  }

  const nsubject = document.getElementById("nsubject");
  const timeb = document.getElementById("timeb");
  const prevbtn = document.getElementById("prevbtn");
  const choosebox = document.getElementById("choosebox");

  if (nsubject) {
    nsubject.classList.add("remove");
  }

  if (timeb) {
    timeb.classList.add("remove");
  }

  if (prevbtn) {
    prevbtn.classList.add("remove");
  }

  if (choosebox) {
    choosebox.classList.add("remove");
  }

  let html = `
    <h2 id="red">
      Your Score: ${score} / ${questions.length}
    </h2>

    <p>
      Time Taken: ${formatTime(timeTaken)}
    </p>

    <hr>
  `;

  questions.forEach((q, i) => {
    html += `
      <h3 style="margin-top:30px;margin-bottom:20px">
        Question ${i + 1}
      </h3>

      <p>${q.question}</p>

      <div class="options">
    `;

    ["a", "b", "c", "d"].forEach((opt) => {
      const isUser = userAnswers[i] === opt;
      const isCorrect = q.answer === opt;

      let className = "";

      if (isCorrect) {
        className = "correct";
      } else if (isUser && !isCorrect) {
        className = "wrong";
      }

      html += `
        <div class="${className}" style="margin-bottom:20px">
          <input
            type="radio"
            disabled
            ${isUser ? "checked" : ""}
          >

          <label>${q.option[opt]}</label>
        </div>
      `;
    });

    html += `
      </div>
    `;
  });

  const questionbody = document.getElementById("questionbody");

  if (questionbody) {
    questionbody.innerHTML = html;
  }

  nextbtn.textContent = "Continue";
  nextbtn.addEventListener("click", () => {
    window.location.href="../../dashboard"
  });

  await savePracticeSession(score, timeTaken);
}

nextbtn.addEventListener("click", () => {
  if (isSubmitted) {
    return;
  }

  nextQuestion();
});

prevbtn.addEventListener("click", () => {
  if (isSubmitted) {
    return;
  }

  prevQuestion();
});

loadQuestions();

function timebox() {
  const circle = document.getElementById("circle");
  const timeText = document.getElementById("timeText");
  const minutesText = document.getElementById("minuteText");

  if (!circle || !timeText || !minutesText) return;

  const radius = 30;
  const circumference = 2 * Math.PI * radius;

  circle.style.strokeDasharray = circumference;

  const totalMinutes = Number(wtime);
  const totalTime = totalMinutes * 60;

  let timeLeft = totalTime;

  function formatTime(seconds) {
    const mins = Math.floor(seconds / 60);
    const secs = seconds % 60;

    return `${String(mins).padStart(2, "0")}:${String(secs).padStart(2, "0")}`;
  }

  function updateTimer() {
    if (isSubmitted) {
      clearInterval(timer);
      return;
    }

    const progress = timeLeft / totalTime;

    circle.style.strokeDashoffset = circumference * (1 - progress);

    timeText.textContent = formatTime(timeLeft);

    const minutesLeft = Math.floor(timeLeft / 60);

    minutesText.textContent = `${minutesLeft} min left`;

    if (timeLeft <= 60) {
      circle.style.stroke = "red";
    }

    if (timeLeft <= 0) {
      clearInterval(timer);

      timeText.textContent = "00:00";
      minutesText.textContent = "0 min left";

      submitTest();

      return;
    }

    timeLeft--;
  }

  updateTimer();

  const timer = setInterval(updateTimer, 1000);
}

document.addEventListener("keydown", function (e) {
  if (isSubmitted) return;

  const tag = document.activeElement?.tagName;

  if (tag === "INPUT" || tag === "TEXTAREA") {
    return;
  }

  if (e.key === "ArrowRight") {
    nextQuestion();
  }

  if (e.key === "ArrowLeft") {
    prevQuestion();
  }
});

let toastContainer;

function createToastContainer() {
  if (toastContainer) return;

  toastContainer = document.createElement("div");
  toastContainer.className = "toast-container";

  document.body.appendChild(toastContainer);
}

function showToast(message, type = "info") {
  createToastContainer();

  const toast = document.createElement("div");

  toast.className = `toast toast-${type}`;
  toast.textContent = message;

  toastContainer.appendChild(toast);

  requestAnimationFrame(() => {
    toast.classList.add("show");
  });

  const duration = type === "success" ? 8000 : 3000;

  setTimeout(() => {
    toast.classList.remove("show");

    setTimeout(() => {
      toast.remove();
    }, 300);
  }, duration);
}
