import { supabase } from "../../../dashboard/js/supabase.js";
...
const wtime = sessionStorage.getItem("wtime");
const WexamType = sessionStorage.getItem("WexamType");
const loadimg = document.getElementById("loader");

const loader = document.getElementById("loading");
const reload = document.getElementById("reload");
const linfo = document.getElementById("linfo");
const tawinfo = document.getElementById("tawinfo");
const nextbtn = document.getElementById("nextbtn");

let isSubmitted = false;
let isSavingSession = false;

const API_URL = `https://questions.aloc.com.ng/api/v2/q?subject=${wsubject}&type=UTME&limit=${wsnq}`;
...
async function loadQuestions() {
  try {
    if (!wsubject || !wsnq) {
      throw new Error("Practice information is missing.");
    }

    const questionCount = Number(wsnq);

    if (!questionCount || questionCount <= 0) {
      throw new Error("Invalid question count.");
    }

    const requests = Array.from({ length: questionCount }, () =>
      fetch(API_URL, {
        method: "GET",
        headers: {
...
function createQuestionBoxes() {
  const container = document.getElementById("questbox");
...
function updateBoxes() {
  const boxes = document.querySelectorAll(".qboxes");
...
function showQuestion() {
  ...
  const html = `
    <h2>
      Question <b>${currentQuestion + 1}</b> of <b>${questions.length}</b>
    </h2>
...
async function savePracticeSession(score, timeTaken) {
  ...
  try {
    ...
    const examType = WexamType || "WASSCE";
    ...
    if (historyError) {
      console.error("Practice history error:", historyError);

      throw new Error(
        `Practice history could not be saved: ${historyError.message}`,
      );
    }
    ...
    if (sessionError) {
      console.error("Practice session error:", sessionError);
    }
    ...
    if (historyFetchError) {
      ...
      throw new Error(
        `Could not load practice history: ${historyFetchError.message}`,
      );
    }
    ...
    if (statsFetchError) {
      ...
      throw new Error(
        `Could not load leaderboard stats: ${statsFetchError.message}`,
      );
    }
    ...
    if (existingStats) {
      ...
      if (statsUpdateError) {
        ...
        throw new Error(`Could not update XP: ${statsUpdateError.message}`);
      }
    } else {
      ...
      if (statsInsertError) {
        ...
        throw new Error(
          `Could not create XP stats: ${statsInsertError.message}`,
        );
      }
    }
    ...
    for (const [subject, subjectXP] of Object.entries(subjectXPMap)) {
      ...
      if (existingSubject) {
        ...
        if (subjectInsertError) {
          console.error(
            `Failed to create ${subject} leaderboard:`,
            subjectInsertError,
          );
        }
      }
    ...
    showToast(error.message || "Progress could not be saved.", "error");
  } finally {
...
async function submitTest() {
  ...
  const nsubject = document.getElementById("nsubject");
  const timeb = document.getElementById("timeb");
  const prevbtn = document.getElementById("prevbtn");
  const choosebox = document.getElementById("choosebox");

  if (nsubject) {
    nsubject.classList.add("remove");
  }

  if (timeb) {
    timeb.classList.add("remove");
  }
  ...
  if (choosebox) {
    choosebox.classList.add("remove");
  }
  ...
  questions.forEach((q, i) => {
    ...
    ["a", "b", "c", "d"].forEach((opt) => {
      const isUser = userAnswers[i] === opt;
      ...
      html += `
        <div class="${className}" style="margin-bottom:20px">
          <input
            type="radio"
            disabled
            ${isUser ? "checked" : ""}
...
function timebox() {
  ...
  const minutesText = document.getElementById("minuteText");

  if (!circle || !timeText || !minutesText) return;
  ...
  const totalMinutes = Number(wtime);
  ...
  function updateTimer() {
    ...
    minutesText.textContent = `${minutesLeft} min left`;
    ...
    if (timeLeft <= 0) {
      ...
      minutesText.textContent = "0 min left";
